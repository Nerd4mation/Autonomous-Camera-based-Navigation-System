...
class World(object):
    ...
    def restart(self):
        ...
        #self.camera_manager2.set_sensor(1, notify=False)
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        global start, steering_value, throttle_value, speed_value, brake_value #added
        self = weak_self()
        if not self:
            return
        ...
        if self.recording:
            if time.time() - start >= 0.1:
                start = time.time()
                image.save_to_disk('_out/%08d_%.2f_%04f_%01d.png' %(image.frame_number, throttle_value, steering_value, brake_value))
