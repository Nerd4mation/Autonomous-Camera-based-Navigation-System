...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        global prediction
        #control_a7a = float(prediction[0][0])
        #print(control_a7a)
        self._control.throttle = float(prediction[0][0])
        ...
        self._control.steer = float(prediction[0][1])
        self._control.brake = float(prediction[0][2])
...
class HUD(object):
    ...
    def tick(self, world, clock):
        global steering_value, throttle_value, speed_value, brake_value
        ...
        self._info_text = [
            ...
            '']
        #########################
        speed_value = 3.6 * math.sqrt(v.x ** 2 + v.y ** 2 + v.z ** 2)
        steering_value = c.steer
        brake_value = c.brake
        throttle_value = c.throttle
...
K.clear_session()
model = tf.keras.models.load_model('Final_Model1', custom_objects={'throttle_mse':throttle_mse, 'steer_mse':steer_mse, 'brake_mse':brake_mse})
...
class CameraManager(object):        
    ...
    def _parse_image(weak_self, image):
        global model, start, steering_value, throttle_value, speed_value, brake_value, prediction #added
        self = weak_self()
        if not self:
            return
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            array2 /= 255
            array2 = cv2.resize(array2, (224,224))
            array2 = np.expand_dims(array2, axis = 0)
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            prediction = model.predict(array2)
            #throttle
